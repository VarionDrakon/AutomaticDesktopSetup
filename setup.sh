...
# Step 1
printf "\033[93mWarning! Did you set the system name and \033[92medit the variables in the setup.sh file\033[93m before running the script? Enter the command: \033[92m{ hostnamectl set-hostname client.domain.name } \033[93mAnd also install your IPA server as a DNS server on your network adapter!\033[0m\n"
...
if apt update -y; then
    ...
    if apt install tree freeipa-client -y; then
        printf "[Step 1.!] \033[93mThe packages: \033[92m[tree freeipa-client ] \033[93mwere installed successfully!\033[0m\n[Step 1.1] Enter the command: \033[92m{ ipa-client-install --mkhomedir --enable-dns-updates } \033[0mfor configure the client after finished this file!\n"
    fi
else
